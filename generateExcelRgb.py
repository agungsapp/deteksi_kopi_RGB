...
def process_all_grades(dataset_path, output_base_path, limit_images):
    for dataset_type in ['train', 'test']:
        for grade in ['A', 'B', 'C']:
            input_path = os.path.join(dataset_path, dataset_type)
            output_path = os.path.join(output_base_path, dataset_type, f"{grade}.xlsx")
            
            results = process_dataset(input_path, grade, limit_images)
            save_to_excel(results, output_path)

if __name__ == "__main__":
    dataset_path = r"D:\JOKI\PYTHON\KOPI\DATASET"
    output_base_path = r"D:\JOKI\PYTHON\KOPI\EXCEL"
    limit_images = 50

    process_all_grades(dataset_path, output_base_path, limit_images)